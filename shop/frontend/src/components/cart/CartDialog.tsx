...
export function CartDialog({ onClose }: CartDialogProps) {
   ...
   const total = cart
      .reduce((acc, item) => acc + item.quantity * item.event.price, 0)
               ...
               {cart.length === 0 ? (
                  ...
                  <>
                     ...
                     <div className="space-y-2 border-t border-zinc-800 pt-4">
                        ...
                        <button className="mt-2 w-full rounded-xl bg-cyan-400 px-5 py-3 font-semibold text-black transition-colors hover:bg-cyan-300">
                            Checkout
                        </button>
